...
\item[\card{Ace}] The following hand must pick up four cards.  The player of the ace must say ``Sorry'', although
  ...
  \item[\card{6}]
    ...
    6_A X_B X_C X_B X_A X_E$.  Note that in this case the net effect is that the order of play has reversed.
...
\begin{itemize}
  ...
  \item
    ...
    Jokers may not be played by themselves, but one or more jokers may be played along with any legal card;
    each joker has the effect of skipping one hand.  For example, if player A plays a Joker and 2, $({J^k}
    2)_A$, C must pick up 2 cards or play a 2 herself; she is of course at liberty to play $({J^k} {J^k} 2)_C$
    to make A pick up 4 cards.  You may butt in on a card played with a joker, but not on the Joker itself
    (the original Jokers are automatically attached to the butted-in card).

    Option: Maybe we should allow butting in on the Joker, which could be exactly equivalent to the butter-in
    playing the original cards, but with an extra Joker.  Question: is it clear when two Jokers are identical?
